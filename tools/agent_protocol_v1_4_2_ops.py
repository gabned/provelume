...
def validate_scope(
    ...
    s = obj(value, "repository pr base_sha head_sha paths_sha256 patch_sha256 patch "
            "actor actor_role authorization_ref decision source observed_at", "scope exception")
    observation(s, now)
    ...
    require(re.fullmatch(rf"https://github[.]com/{re.escape(pr['repository'])}/"
                         rf"(?:pull|issues)/{pr['number']}#issuecomment-[1-9][0-9]*",
                         s["authorization_ref"]) is not None, "unbound authorization reference")
    patch = s["patch"]
...
def sync_vendor(source: Path, target: Path, commit: str, *, check: bool = False) -> dict:
    ...
    lines = ["# Agent Development Protocol 1.4.2 — canonical provenance", "",
             ...
             f"Source: gabned/provelume@{commit}", "",
...
def validate_audit_input(value: Any) -> dict:
    ...
    for raw in rows:
        r = obj(raw, "repository default_branch profile default_sha operations vendor_manifest "
                "vendor_files registry open_campaign_prs unresolved_threads", "audit repository")
        ...
        if r["repository"] == "gabned/nexus":
            require(r["vendor_manifest"] is None and r["vendor_files"] == [],
