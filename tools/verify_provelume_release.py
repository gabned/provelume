...
def _verify_report(
    ...
    *,
    ...
    if payload.get("schema_version") != 1 or payload.get("byte_identical") is not True:
        raise VerificationError(f"{source} is not a green schema-1 report")
    if payload.get("source_repository") != SOURCE_REPOSITORY:
        raise VerificationError(f"unexpected source repository in {source}")
    ...
    declared = _identity_map(payload.get(field), source=f"{source}:{field}")
...
def _verify_build_lock(root: Path, assurance: dict[str, Any], commit: str) -> tuple[str, dict[str, Identity]]:
...
def verify_bundle(
    ...
    *,
    ...
    return {
        ...
        "package_artifacts": [asdict(package_identities[name]) for name in sorted(package_identities)],
