...
offline bundle verifiers were observed successfully. `0.9.0 Lectio` is published under
...
`0.9/S07` was implemented through [#155](https://github.com/gabned/provelume/issues/155) and owner
...
**Current status:** parent tracker #137 records the completed line. `0.9/S01` was completed by the pre-existing
...
active unpublished `0.10.0 — Perceptio` under #160. S01 is the only active implementation slice
through #162 on `product/0.10-s01-universal-representation-contract`; S02–S07 remain planned.
#157 remains closed as not planned and no S08 exists.
...
[`0.10.0 — Perceptio` development plan](releases/0.10.0.md). S01 is active through
[#162](https://github.com/gabned/provelume/issues/162) on
`product/0.10-s01-universal-representation-contract`; no owner pull request exists yet. S02–S07
remain planned. Package, runtime, embedded and Windows identity remain `0.9.0`; Perceptio is
...
| --- | --- | --- | --- | --- |
| `0.10/S01` | Universal representation and support-registry contract | Versioned bundle, anchor and support-level contracts map existing profiles without rewriting them. | No media parser, engine, model, native payload, AI path or migration rewrite. | `active` via #162 |
...
| `0.10/S05` | Video streams, subtitles, scenes, keyframes and frame OCR | Qualified streams combine subtitle, audio, scene, keyframe and selected-frame OCR evidence. | No live capture, DRM bypass, continuous frame OCR, remote inference or generative media. | `planned` |
...
effect and completion criteria. S01 is the sole active implementation slice under #162; no later
slice has an operational issue or owner pull request.
