...
Core ships a versioned, bilingual catalogue. Detection is closed to installed Python distribution
metadata, CPython/platform metadata, executable presence without paths, or explicit external
evidence. Reads perform no process execution, package-manager action, network request or Instance
write. Missing optional evidence is `unverified` or `missing`, never silently inferred.
