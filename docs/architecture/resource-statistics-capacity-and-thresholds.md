...
Status: bounded `0.8/S05` implementation tracked by issue [#130](https://github.com/gabned/provelume/issues/130) and published in the `v0.8.0` Vigilia preview.
...
Each regular directory entry contributes one logical file and its reported `st_size` contributes logical bytes. These values are not deduplicated physical blocks, allocated-block counts or storage reservations. Symlinks and special files are excluded. Before persistence, a stability barrier rechecks the selected membership of every visited directory and the counted type and byte size of every regular file. A concurrent addition, disappearance, file/directory type change or counted-size change is a retryable `resource_statistics_changed` result; unreadable metadata is a bounded local-I/O failure. Timestamp, permission, device and inode churn cannot alter these content-free aggregates and is deliberately not treated as a change, which also lets the scheduler heartbeat replace its own same-sized job record atomically. The explicit file and history bounds fail visibly and are never bypassed by sampling or deletion.
