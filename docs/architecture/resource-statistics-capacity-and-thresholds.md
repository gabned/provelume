...
Each regular directory entry contributes one logical file and its reported `st_size` contributes logical bytes. These values are not deduplicated physical blocks, allocated-block counts or storage reservations. Symlinks and special files are excluded. Before persistence, a stability barrier rechecks the selected membership of every visited directory and the metadata of every counted file. A concurrent addition, disappearance, replacement or metadata change is a retryable `resource_statistics_changed` result; unreadable metadata is a bounded local-I/O failure. The current scheduler job record is the sole exception for timestamp and inode churn caused by its own atomic heartbeat: its regular-file type and counted byte size must still remain stable. The explicit file and history bounds fail visibly and are never bypassed by sampling or deletion.
