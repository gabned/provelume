...
implemented provider-neutral local email identity and intake, and S04 implements replaceable
read-only Gmail/Drive adapters. Published package, runtime, embedded
...
CURRENT_SLICE: 0.9/S04
CURRENT_SLICE_ISSUE: #149
CURRENT_SLICE_BRANCH: product/0.9-s04-gmail-drive-adapters
CURRENT_SLICE_STATE: OWNER_IMPLEMENTATION
NEXT_SLICE: 0.9/S05
...
S04 is owned by [#149](https://github.com/gabned/provelume/issues/149) and adds only:
...
No S01/S02/S03/S04 commit creates a tag, release or asset and none changes package, `__version__`,
...
`0.9/S04` is the current owner implementation under #149. `0.9/S05` is the next forecast only, for
transcript profiles; S06 cross-source qualification and S07 Windows shell/endpoint remain later
Lectio scope. Action Center, AI, RAG, vision/remote OCR and every `0.10+` capability are outside
this record. Port `44851` remains an S07 roadmap decision and is unchanged by S04.
