...
DELIVERED_SLICE_OWNER_PRS: #138,S02_OWNER_PR
NEXT_SLICE: 0.9/S03
NEXT_SLICE_STATE: FORECAST_ONLY
```

GitHub remains authoritative for issue, pull-request and merge state. `S02_OWNER_PR` is replaced
with the exact owner PR number before merge.
...
S02 is owned by [#140](https://github.com/gabned/provelume/issues/140) and adds only:
