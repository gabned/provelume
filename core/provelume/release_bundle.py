...
This module is both importable by the installed application service and copied
verbatim into release bundles as the standalone verifier.
"""
    ...
    for entry in entries:
        if _is_link_like(entry):
            raise VerificationError(
                "release bundle contains a symlink, junction or reparse-point file: "
                f"{entry.name}"
            )
